...
def load_checkpoint(
    ...
    str, Union["RemoveBackgroundPyroModel", pyro.optim.PyroOptim, DataLoader, bool]
]:
...
def load_from_checkpoint(
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if success:
            ...
            logger.info(
                f"Successfully unpacked tarball to {tmp_dir}\n" f"{unpacked_files}"
            )
        else:
        ...
        if force_use_checkpoint or (filebase is None):
            filebase = glob.glob(os.path.join(tmp_dir, "*_model.torch"))[0].replace(
                "_model.torch", ""
            )
        ...
        if "optim" in to_load:
            ...
            scheduler.load(
                filebase + "_optim.pyro", **load_kwargs
            )  # use PyroOptim method
        ...
        if "param_store" in to_load:
            pyro.get_param_store().load(
                filebase + "_params.pyro", map_location=force_device
            )
        ...
        if "dataloader" in to_load:
            ...
            if os.path.exists(filebase + "_train.loaderstate"):
                train_loader = torch.load(
                    filebase + "_train.loaderstate", **load_kwargs
                )
                logger.debug(
                    "Train loader loaded from " + filebase + "_train.loaderstate"
                )
            ...
            if os.path.exists(filebase + "_test.loaderstate"):
                ...
                logger.debug(
                    "Test loader loaded from " + filebase + "_test.loaderstate"
                )
        ...
        if "random_state" in to_load:
            ...
            logger.debug(
                "Loaded random state globally for python, numpy, pytorch, and cuda"
            )
        ...
        if "posterior" in to_load:
            ...
            if os.path.exists(posterior_file):
                ...
                logger.debug(
                    f"Copied posterior file from {posterior_file} to posterior.h5"
                )
            else:
                logger.debug(
...
def attempt_load_checkpoint(
    ...
    try:
        ...
        logger.debug(
            "Unpacked tarball files have a different workflow hash: will not load"
        )
        return {"loaded": False}
